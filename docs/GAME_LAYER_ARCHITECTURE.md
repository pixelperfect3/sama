...
**Phase 2 -- Resolution-independent text (deferred until HiDPI/VR needed):**

Three options, selectable per device tier at runtime behind the same `UiText` API:

| Renderer | Quality | GPU cost | Atlas? | Best for |
|----------|---------|----------|--------|----------|
| **Bitmap** (Phase 1) | Fixed resolution | Trivial | Yes (per size) | Low-end mobile, development |
| **MSDF** | Good at most scales, rounded corners at extreme zoom | Low (1 sample + smoothstep) | Yes (one) | Mid-range mobile |
| **Slug** | Mathematically perfect at any size/angle/DPI | Moderate (Bézier curve eval per pixel) | No | Desktop, VR, high-end mobile |

**MSDF** — Multi-channel signed distance field fonts generated by `msdf-atlas-gen`. Same vertex pipeline as bitmap (one quad per glyph, same atlas). Fragment shader samples the MSDF atlas and reconstructs sharp edges using `fwidth()`. Shader swap, not an architecture change.

**Slug** — Eric Lengyel's GPU algorithm that renders glyphs directly from quadratic Bézier curve data. No atlas needed. Fragment shader evaluates curves per-pixel to determine pixel coverage; vertex shader computes "dynamic dilation" (half-pixel expansion for complete coverage). Produces artifact-free antialiased text at any scale, rotation, or perspective angle — critical for VR where text appears at varying distances and oblique angles.

Key Slug details:
- **Patent:** Dedicated to public domain (March 2026). Reference shaders MIT-licensed on [GitHub](https://github.com/EricLengyel/Slug).
- **bgfx compatible:** Standard vertex + fragment shader pipeline. No compute shaders. Reference HLSL ports to bgfx `.sc` format.
- **Memory:** ~4.8KB per glyph in GPU curve buffers (vs. atlas texture for bitmap/MSDF).
- **Vector graphics:** Same renderer handles text AND vector icons/shapes (SVG, filled/stroked paths). Eliminates separate icon atlas.
- **Implementation:** Port reference HLSL shaders to bgfx `.sc`, implement glyph curve extraction via freetype2, pack curves into GPU textures. ~2-3 weeks with reference code.

**Runtime selection by device tier:**

```cpp
enum class FontRenderer { Bitmap, Msdf, Slug };

// Set at startup based on GPU capability
if (gpuTier >= GpuTier::High)        // Desktop, VR, iPhone 14+, Pixel 8+
    fontRenderer = FontRenderer::Slug;
else if (gpuTier >= GpuTier::Medium)  // Mid-range mobile
    fontRenderer = FontRenderer::Msdf;
else                                   // Low-end / older Android
    fontRenderer = FontRenderer::Bitmap;
```

`UiText` component is renderer-agnostic — it stores text content, font reference, and layout. The rendering backend (bitmap/MSDF/Slug) is selected globally or per-canvas without changing game code.

**Mobile platform guidance:**

| Platform | Recommended | Rationale |
|----------|------------|-----------|
| Desktop | Slug | Plenty of GPU headroom, best quality |
| Desktop VR | Slug | Only option that looks right at all distances/angles |
| iOS (A15+ / M-series) | Slug | Modern Apple GPUs handle the shader easily |
| Android high-end (Mali-G715+, Adreno 740+) | Slug | Sufficient fill rate |
| Android mid-range | MSDF | Lower fill-rate budget, Slug's per-pixel cost matters |
| Android low-end / older | Bitmap | Minimal GPU cost, predictable battery usage |

**Tradeoff — Slug on mobile:** Slug's per-pixel cost is higher than MSDF because the fragment shader evaluates Bézier curves rather than sampling an atlas. On TBDR GPUs (all modern mobile), the band structure limits curve evaluations per pixel and early-Z helps with overdraw. For typical UI text (small to medium sizes, not viewport-filling), the cost is acceptable on high-end mobile. For text-heavy screens (chat, inventory lists), profile before shipping.
