...
| Skinned mesh renders as huge stretched/exploded triangles | Shaders compiled with `u_model[1]` because `BGFX_CONFIG_MAX_BONES` was not passed to shaderc | Setting it on the bgfx C++ target alone is **not enough** — `bgfx_shader.sh` is preprocessed by shaderc independently and defaults the array size to 1. Pass `DEFINES BGFX_CONFIG_MAX_BONES=128` through `_bgfx_shaderc_parse` in `CMakeLists.txt`'s `compile_shader()` / `compile_shader_pp()`. Verify the compiled shader has `u_model[128]` (e.g. via `strings build/include/generated/shaders/vs_pbr_skinned_mtl.bin.h \| grep u_model`). Regression covered by `tests/screenshot/TestSsAnimation.cpp`. |
