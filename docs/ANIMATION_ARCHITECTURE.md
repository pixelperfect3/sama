...
`BGFX_CONFIG_MAX_BONES` defaults to **1** in upstream `bgfx_shader.sh` (the in-engine `target_compile_definitions(bgfx PUBLIC BGFX_CONFIG_MAX_BONES=128)` raises it to 128 for the runtime library). The override **must also be threaded into shaderc** when shaders are compiled — `bgfx_shader.sh` is preprocessed by shaderc independently of the runtime library, and without the define it expands `uniform mat4 u_model[BGFX_CONFIG_MAX_BONES]` to `u_model[1]`. The result is a silent skinning bug: `bgfx::setTransform(bones, 24)` uploads matrices into a 1-slot array and the vertex shader's `u_model[int(a_indices.x)]` reads garbage for any vertex whose bone index is > 0 — vertices fly off in random directions instead of producing a posed mesh. Pass `DEFINES BGFX_CONFIG_MAX_BONES=128` through `_bgfx_shaderc_parse` in `CMakeLists.txt`'s `compile_shader()` / `compile_shader_pp()` helpers (verified in commit `7d90fde`). Each `u_model[i]` is a `mat4` (4 vec4 uniforms), so 128 bones = 512 vec4 uniforms, which fits within bgfx's default `BGFX_CONFIG_MAX_UNIFORMS = 512`. The skinned-rendering screenshot test (`tests/screenshot/TestSsAnimation.cpp`) locks this in: it loads `Fox.glb`, samples a clip at a fixed `playbackTime`, and golden-compares the rendered skinned mesh — any future regression that re-introduces `u_model[1]` will produce a visibly broken golden diff.
