...
- [ ] **bgfx forced single-threaded mode hides CPU↔GPU parallelism on mobile.** `engine/rendering/Renderer.cpp:47-50` calls `bgfx::renderFrame()` once before `bgfx::init()` for any non-headless renderer, which prevents bgfx from spawning its own render thread.  Consequence: command-buffer build + GPU acquire/submit/present all run on the game thread, and `bgfx::frame()` blocks until the GPU's swapchain image is free.  perf_smoke deliberately does NOT measure `bgfx::frame()` wall time, so its budget table shows the engine systems doing fine while the real per-frame cost on Pixel 9 (rolling-ball game, ~2.6 MP fragment work at 1080×2424) is GPU wait time charged to the game thread.  *Fix path:* (1) expose `EngineDesc::singleThreaded = false` default, keep `true` for the screenshot fixture which needs deterministic frame capture; (2) audit `engine/assets/AssetManager` async upload callbacks for any `bgfx::*` calls that happen off the game thread (they would need to be marshalled back); (3) measure on Pixel 9 with a `chrono::steady_clock` bracket around the `bgfx::frame()` call in `Renderer::endFrame` *first* to confirm the wait-time hypothesis before committing to the refactor. The screenshot tests must continue to work in single-thread mode (they assert pixel-exact goldens and rely on synchronous readback).  Going further to per-thread command recording via `bgfx::Encoder` is a much larger refactor and not needed for the current games.  Lower priority than `LightClusterBuilder` was, but **higher leverage than `FrustumCullSystem` NEON** for any game with non-trivial GPU work.
