...
- [x] Lifecycle handling
  ...
  - **Cross-launch state persistence:** NativeActivity does not surface the Java `onSaveInstanceState` / `onRestoreInstanceState` callbacks to native code. Games use the file-based API in `engine/platform/android/AndroidSavedState.{h,cpp}` instead — `Engine::registerSaveStateCallback(cb)` wires a serialiser into `APP_CMD_SAVE_STATE` and `engine::platform::readSavedState(name)` / `writeSavedState(name, bytes)` read/write small blobs under `android_app::activity->externalDataPath` (equivalent to `Context.getExternalFilesDir(null)`, no `WRITE_EXTERNAL_STORAGE` permission required). The path is cached at `initAndroid` time so the helpers also unit-test cleanly on the host (see `tests/platform/TestAndroidSavedState.cpp`).
...
- [x] Audio backend
  ...
  - **Race-safe AAudio callback** (commit `0e9bf2e`): our vendored `third_party/soloud_patches/soloud_miniaudio.cpp` gates the SoLoud-mix callback on a `std::atomic<bool> gAudioReady` flag — set true after `postinit_internal` returns, checked acquire-load on every callback. Without the gate, miniaudio's AAudio backend would start the playback callback inside `ma_device_init` (before SoLoud allocates `mResampleData[]`), causing a 100%-reproducible SIGSEGV in `mapResampleBuffers_internal` on Pixel 9 / Android 16. See "Android audio: race-safe AAudio gate" in NOTES.md for the full rationale.
...
- [x] Gyroscope / accelerometer input
  ...
  - **Opt-in via `EngineDesc::enableGyro = true`.**  Default is `false` — the sensor pair burns ~5-10 mW continuously even when the game never reads it, so games must explicitly opt in.  `Engine::initAndroid` and `APP_CMD_RESUME` both gate sensor-enable on the latched opt-in choice (PAUSE → RESUME no longer silently re-enables for games that didn't ask).  See audit item #P1 + `docs/NOTES.md` "Android gyro/accel — opt-in + 30 Hz".
  - Default sample rate is 30 Hz (`AndroidGyro::sampleRateUs_ = 33333`).  Games that need higher rates (FPS look, AR pose) can crank `sampleRateUs_` before `init()`.  The iOS path intentionally stays at 60 Hz — CoreMotion's sensor fusion loop runs continuously regardless of poll interval, so the Android-specific power win doesn't apply there.
...
- [x] **`LightClusterBuilder` dominated per-frame CPU on Pixel 9 (22 ms of 26 ms total).** Three `bgfx::updateTexture2D` calls per frame (lightData + lightGrid + lightIndex), unconditional, even when no light or camera moved. On real Vulkan the per-upload staging-buffer + transfer-queue path is ~6-7 ms each. The CPU compute (2 passes of O(3456 clusters × N lights)) is much smaller. *Two-step fix:* (1) dirty-flag the three texture uploads so they only fire when light data or cluster geometry changed; (2) cache cluster AABBs across frames when the projection matrix is unchanged. *Fix landed (commit `9b4f123`, 2026-05-22):* `engine/rendering/LightClusterBuilder.{h,cpp}` now hashes `(projMatrix, nearPlane, farPlane)` with `std::memcmp` and skips the 3,456-entry AABB rebuild when those inputs are bit-identical to the previous frame.  Each texture upload has its own dirty bit — `lightData` invalidates on a FNV-1a hash mismatch over the live `lights_[]` bytes; `lightGrid` + `lightIndex` invalidate together (they must stay coherent on the GPU) when either the lights OR cluster geometry changed.  *Measured before / after on physical Pixel 9 hardware, same `perf_smoke` APK rebuilt against the new code:* LightClusterBuilder mean **22.29 ms → 0.056 ms** (398× reduction), frame total mean **26.63 ms → 4.175 ms / p99 5.7 ms** — comfortably inside the 16.67 ms 60-fps budget with ~10 ms headroom.  Desktop M3 mirror: 0.147 ms → 0.003 ms (~50×).  Tests: `tests/rendering/TestLighting.cpp` adds 5 cases asserting zero-upload on identical inputs, exactly-3 uploads on moved-light, AABB rebuild on projection change, near/far cache invalidation, and zero-light handling.  Caveat for moving-camera games: AABB cache still hits (projection rarely changes) but lightData hash misses every frame because lights live in view space — so uploads still fire most frames.  Static-camera and idle scenes (menus, paused gameplay) get the full 398× win.
- [x] **bgfx forced single-threaded mode hides CPU↔GPU parallelism on mobile.** *Symptom (before fix):* `engine/rendering/Renderer.cpp` called `bgfx::renderFrame()` once before `bgfx::init()` for every non-headless renderer, preventing bgfx from spawning its own render thread.  Command-buffer build + GPU acquire/submit/present all ran on the game thread, so `bgfx::frame()` blocked until the GPU's swapchain image was free.  Reporter's measurement on a rolling-ball game at 1080×2424 on Pixel 9 (Tensor G4): 21.20 ms total frame time, 20.50 ms of it inside `bgfx::frame()` — pure swapchain wait charged to the game thread.  *Fix landed (2026-05-24):* `EngineDesc::singleThreaded` defaults to `false` (multi-threaded), and `Renderer::init` now only calls `bgfx::renderFrame()` when `desc.singleThreaded == true`.  All three platform branches in `Engine.cpp` (desktop / Android / iOS) forward `desc.singleThreaded` into `RendererDesc`.  `AssetManager` audit (`engine/assets/AssetManager.h:36-52`) confirmed every `bgfx::*` call lives inside `processUploads()`, which the contract already pins to the main thread — no marshalling changes were needed.  Screenshot fixture (`tests/screenshot/ScreenshotFixture.cpp`) keeps its own pre-init `bgfx::renderFrame()` call because it never goes through `Renderer` and its blit-and-readback flow needs deterministic single-threaded capture; unaffected by the new flag.  `apps/perf_smoke` learned `--single` / `--multi` flags + a `run_both.sh` wrapper that runs both back-to-back for the comparison.  Unit-level plumbing covered by `tests/rendering/TestThreadingMode.cpp` (4 cases: default values + both flag values init/shutdown cleanly in headless mode).  *Expected delta on Pixel 9 sample game:* `bgfx::frame()` drops from ~20 ms to ~0.1 ms on the game thread (the render thread now owns the vsync wait), bringing the rolling-ball game from ~47 fps to a comfortable 60 fps with margin.  Going further to per-thread command recording via `bgfx::Encoder` is a much larger refactor and not needed for the current games.
- [ ] **`FrustumCullSystem` is a NEON SIMD candidate.** Per-entity work today is `~15` scalar ops × 6 planes × N entities (positive-vertex AABB test in `engine/math/Frustum.h::containsAABB`). On Pixel 9 with 702 entities the system measures 0.72 ms (over its 0.30 ms budget) and the cost is dependent FP math — Cortex-X4 throughput is ~1/3 of Apple Silicon for this pattern. *Fix path (best → worst effort):* (a) transpose plane storage to `planeX[6] / planeY[6] / planeZ[6] / planeW[6]` and use `vbslq_f32` to remove the per-component branch in `positive = (plane.xyz >= 0 ? max : min)` — collapses 6 plane tests to ~15 NEON ops (was ~90 scalar); (b) batch 4 entities per pass to fill `float32x4_t` registers. (a) alone recovers ~0.4 ms / frame on Pixel 9; (a)+(b) recovers ~0.55 ms. **Lower priority than LightClusterBuilder** (the recovered time is 3% of the bigger fix's gain), but a clean win once the cluster problem is closed.
