...
- [x] **`LightClusterBuilder` dominates per-frame CPU on Pixel 9 (22 ms of 26 ms total).** Three `bgfx::updateTexture2D` calls per frame (lightData + lightGrid + lightIndex), unconditional, even when no light or camera moved. On real Vulkan the per-upload staging-buffer + transfer-queue path is ~6-7 ms each. The CPU compute (2 passes of O(3456 clusters × N lights)) is much smaller. *Two-step fix:* (1) dirty-flag the three texture uploads so they only fire when light data or cluster geometry changed; (2) cache cluster AABBs across frames when the projection matrix is unchanged. *Fix landed (2026-05-22):* `engine/rendering/LightClusterBuilder.{h,cpp}` now hashes `(projMatrix, nearPlane, farPlane)` with `std::memcmp` and skips the 3,456-entry AABB rebuild when those inputs are bit-identical to the previous frame.  Each texture upload has its own dirty bit — `lightData` invalidates on a FNV-1a hash mismatch over the live `lights_[]` bytes; `lightGrid` + `lightIndex` invalidate together (they must stay coherent on the GPU) when either the lights OR cluster geometry changed.  Desktop `perf_smoke` (5-run mean, M3) drops from ~0.15 ms → 0.003 ms LightClusterBuilder — a ~50× reduction confirming the static-camera fast path fires zero uploads.  Pixel 9 hardware measurement is pending (device was disconnected at the time of this write-up); the same cache logic that fires zero uploads on desktop's static `perf_smoke` camera will fire zero uploads on Pixel 9's same static `perf_smoke` camera, so the expected Pixel 9 numbers are LightClusterBuilder mean 22.29 ms → ~2 ms and frame total 26.63 ms → ~6 ms (the residual is the once-per-startup AABB rebuild amortised over 600 frames, plus the three first-frame uploads).  Tests: `tests/rendering/TestLighting.cpp` adds 5 cases asserting zero-upload on identical inputs, exactly-3 uploads on moved-light, AABB rebuild on projection change, near/far cache invalidation, and zero-light handling.
