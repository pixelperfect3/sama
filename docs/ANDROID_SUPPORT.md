...
- [ ] **`LightClusterBuilder` dominates per-frame CPU on Pixel 9 (22 ms of 26 ms total).** Three `bgfx::updateTexture2D` calls per frame (lightData + lightGrid + lightIndex), unconditional, even when no light or camera moved. On real Vulkan the per-upload staging-buffer + transfer-queue path is ~6-7 ms each. The CPU compute (2 passes of O(3456 clusters × N lights)) is much smaller. *Two-step fix:* (1) dirty-flag the three texture uploads so they only fire when light data or cluster geometry changed; (2) cache cluster AABBs across frames when the projection matrix is unchanged. Expected: 22 ms → ~2 ms on Pixel 9 = back to 60 fps in `perf_smoke`. Measured by `apps/perf_smoke` running on a Pixel 9 (5-run mean: 22.29 ± 0.56 ms LightClusterBuilder). **The current rolling-ball sample game does NOT trigger this** because it has no `PointLightComponent` / `SpotLightComponent` and never calls `LightClusterBuilder::update` — but any game that adds dynamic point lights immediately pays the full cost.
