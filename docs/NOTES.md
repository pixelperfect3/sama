...
- [ ] **Mali-G715 (Pixel 9) shadow-comparison driver bug.** Shadow rendering works correctly on every other tested target (Metal desktop, Android emulator with gfxstream Vulkan via MoltenVK on M3) but produces NO visible shadow on Pixel 9's Mali-G715. Verified with the `apps/android_test/AndroidTestGame` helmet-on-ground scene: helmet's `ShadowVisibleTag` is set, mesh has 14556 verts / 46356 indices, world position correct, shadow atlas handle valid, shadow matrix non-NaN, helmet projects to sensible UV coords. Same APK shows clear helmet-shaped cast shadow on the emulator. Tried unsuccessfully on Mali: `D24S8` vs `D32F` shadow format, toggling Y-flip in the bias matrix, `SampleCmp` (implicit LOD, `OpImageSampleDrefImplicitLod`) vs `SampleCmpLevelZero` (explicit LOD, `OpImageSampleDrefExplicitLod`) — none of these change the result. *Why:* depth comparison sampling on Mali ARM Vulkan drivers is the most plausible culprit; this is a known fragile area. *Path forward:* implement an alternative shadow technique that doesn't rely on hardware depth comparison (variance shadow maps, exponential shadow maps, or screen-space shadows). *Workaround:* test shadow content on the emulator; ship without dynamic shadows on Mali devices, or use baked lightmaps.

- [ ] **physics_demo crashes in `stbtt_GetGlyphShape`** during `ImGui::NewFrame()` → `ImFontBaked_BuildLoadGlyph`. ImGui 1.92 (bundled with the new bgfx) uses lazy font baking via `ImGuiBackendFlags_RendererHasTextures` and calls `stbtt_GetGlyphShape` with corrupt compound-glyph data (faults reading address `0xff000000` at `+2680` in the function). The bgfx wrapper provides stb_truetype with bx allocators (`USE_LOCAL_STB=1`); `imconfig.h` correctly defines `IMGUI_DISABLE_STB_TRUETYPE_IMPLEMENTATION` so there's only one copy. *Why it's puzzling:* `hierarchy_demo` and `animation_demo` use the exact same `ImGui::Begin(...)` pattern through the same `engine_core`/`engine_debug` libraries and do NOT crash — only physics_demo does. Reproducible after a clean rebuild. Likely a heap layout / object lifetime difference triggered by Jolt allocations. *Path forward:* either (a) bisect by stripping the physics setup down to find what corrupts memory, or (b) wait for upstream bgfx to ship a known-good ImGui 1.92 integration. *Workaround in the meantime:* DebugHud already provides the HUD overlay; ImGui panel is non-essential.
