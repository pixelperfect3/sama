...
- [ ] **physics_demo + animation_demo crash in `stbtt_GetGlyphShape`** during `ImGui::NewFrame()` → `ImFontBaked_BuildLoadGlyph`. ImGui 1.92 (bundled with the new bgfx) uses lazy font baking via `ImGuiBackendFlags_RendererHasTextures` and calls `stbtt_GetGlyphShape` with corrupt compound-glyph data (faults reading address `0xff000000` at `+2680` in the function). The bgfx wrapper provides stb_truetype with bx allocators (`USE_LOCAL_STB=1`); `imconfig.h` correctly defines `IMGUI_DISABLE_STB_TRUETYPE_IMPLEMENTATION` so there's only one copy. *Update 2026-04-27:* `animation_demo` also crashes with this exact backtrace at startup — `ImGui::Text` → `CalcTextSize` → `ImFontBaked_BuildLoadGlyph` → `stbtt_MakeGlyphBitmapSubpixel` → fault. Verified the crash exists on `main` HEAD before my AssetManager/AnimationSystem perf changes (stashed and rebuilt original — same `EXC_BAD_ACCESS` at `stbtt_GetGlyphShape + 2680`). `hierarchy_demo` still runs fine, so it's not universal. The earlier hypothesis that this was Jolt-specific is wrong — animation_demo doesn't use physics. Likely heap-layout sensitive: anything that allocates "enough" before `ImGui::NewFrame()` trips it. *Path forward:* either (a) bisect by stripping the demo to find the trigger threshold, or (b) wait for upstream bgfx to ship a known-good ImGui 1.92 integration. *Workaround in the meantime:* DebugHud already provides the HUD overlay; ImGui panel is non-essential.
