...
- [~] **physics_demo + animation_demo crash in `stbtt_GetGlyphShape` — workaround applied 2026-04-27 (root cause not isolated).** ImGui 1.92's lazy font baker faults at `stbtt_GetGlyphShape + 2680` on the FIRST `ImGui::Text` (or any first-glyph load) inside these two demos. Crash address varies (`0x50002`, `0xff000000`, `0x0`) which is the giveaway: the value being dereferenced is uninitialized/stomped, not a single bad pointer. lldb backtrace pinpoints the failing instruction inside the **TrueType compound-glyph branch** of `stbtt_GetGlyphShape` — specifically the `STBTT_malloc` call at `stb_truetype.h:1872` (`tmp = STBTT_malloc((num_vertices+comp_num_verts)*sizeof(stbtt_vertex), info->userdata)`) for compound glyphs. `STBTT_malloc` is wired to bgfx's `memAlloc` → `bx::alloc(s_ctx.m_allocator, _size)` (imgui.cpp:587). Asm at `+2648/+2652` shows x26 being loaded then re-loaded as a vtable pointer — the value loaded is garbage from `_MergedGlobals + 1008`, NOT from `s_ctx.m_allocator` (verified: the allocator pointer at `s_ctx + 0x8` is intact, watchpoint never fires). **Why `hierarchy_demo` and `helmet_demo` survive:** unclear — they call `ImGui::Text`/`ImGui::Begin` in identical patterns. Heap layout / link order seems to influence whether codegen picks up the wrong global. The earlier "Jolt-specific" hypothesis is wrong (animation_demo has no Jolt; ik_demo and audio_demo also crash with the same signature; hierarchy_demo and helmet_demo still work). **Workaround taken:** ripped all `imgui.h` calls out of `physics_demo/main.mm`, `physics_demo/PhysicsGame.cpp`, and `animation_demo/main.mm`; their HUD content is now drawn through `engine::ui::DebugHud`. `physics_demo`'s panel was 100% read-only so nothing is lost. `animation_demo`'s panel had real interactivity (Pause/Stop buttons, Speed/Scrub/IK sliders, Instance radio buttons, Blend trigger, IK toggle, SM Speed) — re-bound to keys: Space=play/pause, S=stop, Tab=next instance, Up/Down=speed, Comma/Period=scrub, B=blend, I=IK toggle, J/K=IK weight, X=reset all, R=full reset, Left/Right=SM speed, 1-9=clip, H=HUD toggle. **Tradeoffs:** (1) IK target X/Y/Z position sliders dropped — the demo still toggles IK on/off and shows the effector working at default position, but you can't drag the target around. Adding 6 more keys for ±X/Y/Z would clutter the keymap; if interactive target movement matters, prefer fixing ImGui (option below). (2) `pixhud` is read-only so live editing widgets like the colored cube legend in `physics_demo` become plain text rows. (3) `hierarchy_demo` and `helmet_demo` STILL use ImGui and STILL work — keeping the engine's `imguiCreate`/`imguiBeginFrame`/`imguiEndFrame` wiring intact was important so as not to break them. **Path forward to a real fix:** the crash is reproducible after a clean rebuild and is in upstream bgfx's bundled `stb_truetype.h` compound-glyph path with bx allocators. Options ranked by effort: (a) replace the embedded Roboto/Kenney/FontAwesome fonts in `bgfx/examples/common/imgui/imgui.cpp` with `io.Fonts->AddFontDefault()` (ProggyClean — bitmap, no compound glyphs, no stb_truetype path) — would fix all demos but requires patching bgfx; (b) build with AddressSanitizer to confirm whether the corruption is heap-stomp or codegen, then track upstream; (c) wait for upstream bgfx to ship an updated ImGui 1.92 integration. **Pre-existing collateral:** `ik_demo`, `ik_hand_demo`, and `audio_demo` also crash at startup with the same backtrace on `main` (verified before my changes via `git stash`). They were not in scope for this task and remain broken. They're prime candidates for the same workaround if interactivity isn't critical.
