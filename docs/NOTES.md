...
- [ ] **Default MSDF atlas.** Run `brew install msdf-atlas-gen` then the command in `assets/fonts/README.md` to generate `ChunkFive-msdf.{png,json}` from the bundled `ChunkFive-Regular.ttf`. Until this is done, `MsdfFont::loadFromFile` returns false and the bitmap default is used. *Why:* the MSDF code path is untested against a real atlas; we want to verify the metric layout and shader scaling before promoting MSDF as the default.
