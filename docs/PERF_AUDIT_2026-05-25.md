...
- [x] **(#cull-aabb) FrustumCullSystem.cpp:41-43 + ShadowCullSystem.cpp:42-44** — was `glm::abs(Vec4)` ×3 + `Mat3` ctor (12-float copy) + `Mat3 * Vec3` per entity.  **Landed:** extracted the shared `computeConservativeWorldAabb()` helper into `engine/rendering/systems/CullHelpers.h`; 9 `fabsf` + 9 muls + 6 adds, no temporary objects on the stack.  Both cull systems now call into the helper so any future precision tweak lands in one place.  *trivial → landed.*
- [x] **(#cull-cache) FrustumCullSystem.cpp:50-55** — was `reg.has<VisibleTag>` followed by `emplace`/`remove` — two sparse-set hits in the transition cases.  **Landed:** single `has` probe at top of the lambda, cached as `wasVisible`.  Steady-state hot entities (visible last frame, still visible this frame) skip both the `emplace` and the `remove` entirely.  Combined with #cull-aabb above, perf_smoke (702 entities, 600 frames, M-series Mac, 5 runs each side): mean **0.048 ms → 0.034 ms (−29%)** median; best-case after run hit 0.024 (−49%) but that's noise floor.  Audit projection (30 + 40 ns/entity × 702 ≈ 0.049 ms saving) bracketed the measured result.  *low → landed.*
- [x] **(#cull-shadow-multi) ShadowCullSystem.cpp:24-29, 51-69** — used to walk all mesh entities per cascade; 3 cascades = 3× view iteration + 3× AABB rebuild per entity.  **Landed:** new `update(reg, res, std::span<const Frustum>, baseCascadeIdx)` overload walks the mesh view once, builds the world AABB once per entity, then tests against every frustum in the span and accumulates the mask in a local before writing back.  Existing per-cascade overload kept for back-compat — it now delegates to the multi-cascade form with a one-element span so both paths are semantically identical.  Contract: the multi-cascade overload owns the bits in `[baseCascadeIdx, baseCascadeIdx + N)` and preserves bits outside that range (lets a spot-light shadow share the same `ShadowVisibleTag` without interference).  Two new test cases pin the result against the reference per-cascade form (byte-identical mask) and the "preserves bits outside owned range" contract.  Not measured on perf_smoke (the scene doesn't drive shadow cull) — audit projection of ~66% reduction with 3 cascades is the math, not a measurement.  All 22 screenshot tests pass — visual output unchanged.  *medium → landed.*
