...
9. [x] Bloom defaults to 5 downsample steps on Mid tier — 10 fullscreen passes; Mid should cap at 3 (#T3) — **landed: TierConfig::bloomDownsampleSteps (low=0/mid=3/high=5) actually reaches PostProcessSystem now. Two-part fix because the field was inert before: (a) tierToRenderSettings forwards the value with a 0..5 clamp; (b) submit() switched from resources_.steps() (init-time constant) to settings.bloom.downsampleSteps as the loop count**
